...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="workspace-stack">
      <section className="workspace-card project-hero-card"><div><span className={`status-pill status-${project.status.toLowerCase()}`}>{project.status.replaceAll("_", " ")}</span><h2>{project.title}</h2><p>{project.summary}</p></div><div className="progress-orb"><strong>{progress}%</strong><span>overall progress</span></div></section>
      ...
      {project.links.length > 0 && <section><div className="section-title-small"><span className="kicker">IMPORTANT LINKS</span><h2>Project resources</h2></div><div className="resource-grid">{project.links.map((link) => <a className="workspace-card resource-link" href={link.url} target="_blank" rel="noreferrer" key={link.id}>{link.label}<ExternalLink size={17} /></a>)}</div></section>}
      <section><div className="section-title-small"><span className="kicker">UPDATES</span><h2>Notes from the studio</h2></div>{project.updates.length ? <div className="timeline">{project.updates.map((update) => <article key={update.id}><time>{update.publishedAt?.toLocaleDateString("en-LK", { dateStyle: "medium" })}</time><div className="workspace-card"><h3>{update.title}</h3><p>{update.body}</p></div></article>)}</div> : <div className="workspace-empty compact"><p>Published project updates will appear here.</p></div>}</section>
    </div>
  );
}
